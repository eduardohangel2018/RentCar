...
class CarRepository private constructor(context: Context) {
    ...
    private var mAppDatabase: AppDatabase = AppDatabase(context)
    ...
    fun save(car: CarModel): Boolean {
        return try {
            val db = mAppDatabase.writableDatabase
            ...
            contentValues.put(DatabaseConstants.CARS.COLUMNS.DESCRIPTION, car.desc)
    ...
    fun get(id: Int): CarModel? {
        ...
        return try {
            val db = mAppDatabase.readableDatabase

            val projection = arrayOf(
                DatabaseConstants.CARS.COLUMNS.NAME,
                DatabaseConstants.CARS.COLUMNS.MODEL,
                DatabaseConstants.CARS.COLUMNS.DESCRIPTION,
            ...
            if (cursor != null && cursor.count > 0) {
                ...
                val description = cursor.getString(cursor.getColumnIndex(DatabaseConstants.CARS.COLUMNS.DESCRIPTION))
                ...
                car = CarModel(id, name, model, price, description, category, status)
            }
        ...
        return try {
            ...
            if (cursor != null && cursor.count > 0) {
                while (cursor.moveToNext()) {
                    ...
                    val name = cursor.getString(cursor.getColumnIndex(DatabaseConstants.CARS.COLUMNS.NAME))
                    val model = cursor.getString(cursor.getColumnIndex(DatabaseConstants.CARS.COLUMNS.MODEL))
                    val desc = cursor.getString(cursor.getColumnIndex(DatabaseConstants.CARS.COLUMNS.DESCRIPTION))
                    val price = cursor.getString(cursor.getColumnIndex(DatabaseConstants.CARS.COLUMNS.PRICE))
                    val category = cursor.getString(cursor.getColumnIndex(DatabaseConstants.CARS.COLUMNS.CATEGORY))
                    val status = (cursor.getInt(cursor.getColumnIndex(DatabaseConstants.CARS.COLUMNS.STATUS)) == 1)
    ...
    fun getAvailable(): List<CarModel> {
        ...
        return try {
            val db = mAppDatabase.readableDatabase
            val cursor = db.rawQuery("SELECT id, name, model, description, price, category WHERE status = 1", null)
    ...
    fun update(car: CarModel): Boolean {
        return try {
            ...
            values.put(DatabaseConstants.CARS.COLUMNS.DESCRIPTION, car.desc)
